...
bool Kernel::InitCL()
{
	...
	cl_command_queue_properties* properties = new cl_command_queue_properties(CL_QUEUE_PROFILING_ENABLE);
	queue = clCreateCommandQueueWithProperties(context, devices[deviceUsed], properties, &error);
	if (!CHECKCL(error)) return false;
	// create a second command queue for asynchronous copies
	queue2 = clCreateCommandQueueWithProperties(context, devices[deviceUsed], properties, &error);
